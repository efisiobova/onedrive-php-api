...
//echo "|".$url; return;
    ...
    {
        $objectId = null !== $objectId ? $objectId : 'drive/root'; //'me/skydrive';  /children
        $result   = $this->apiGet($objectId);

        if (isset($result->folder)) echo "FOLDER";
        if (isset($result->file)) echo "FILE";


        \common\models\Tool::print_r($result); return;
    ...
    public function shareObject($objectId, $properties = array(), $temp = false)
    {
      ...
      $this->apiPost($objectId."/action.invite", (object) $properties);
